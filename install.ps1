param(
  [string]$Destination = $(if ($env:CODEX_HOME) { Join-Path $env:CODEX_HOME 'skills\docs-from-code' } else { Join-Path $HOME '.codex\skills\docs-from-code' })
)
...
Write-Host '已安装 docs-from-code ->' $Destination